...
- тесты запускай через ./run_all_tests.sh (собирает во временной копии и не трогает target/); вручную: mvn test -DforkCount=8 -DreuseForks=true -DthreadCount=4 (ramdisk не использовать)
- НИКОГДА не запускай mvn clean в этом проекте: пользователь запускает приложение из target/, там лежат config/ (настройки, API-ключ) и projects/ (файлы романов). Сборка JAR: mvn package -DskipTests
- таймауты команд в работе — вдвое меньше прежних: не более 150 c на одиночную сборку/команду и 300 c на прогон тестов (внутренние таймауты iceja-writer не трогать)
...
- [2026-09-18] Проблема: `mvn clean` удалил `target/` вместе с `config/app.json` и `projects/*.ijw`, а у открытого приложения исчез JAR → `NoClassDefFoundError` при ленивой загрузке классов. Решение: сборка только `mvn package` без clean; тесты — через `./run_all_tests.sh` во временной копии.
### Критерии завершения
